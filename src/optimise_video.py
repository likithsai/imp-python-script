...
class VideoConverter:
    ...
    def convert_video(self, video_path: Path, index: int, total: int) -> None:
        ...
        if duration <= 0:
            print(f"\r{Constants.CLR}{prefix} | {Constants.RED}FAIL{Constants.RESET}")
            return
        ...
        cmd = [
            ...
            str(video_path),
            "-c:v",
            Constants.VIDEO_CODEC,
            "-preset",
            Constants.PRESET,
            "-crf",
            Constants.CRF,
            "-c:a",
            Constants.AUDIO_CODEC,
            "-b:a",
        ...
        try:
            ...
            for line in process.stderr:
                if "time=" in line:
                    t = next(
                        (p.split("=")[1] for p in line.split() if p.startswith("time=")),
                        "00:00:00",
                    )
            ...
            if process.returncode == 0:
                ...
                if new_size < original_size:
                    ...
                    status = (
                        f"{Constants.GREEN}DONE{Constants.RESET} "
                        f"({self.format_size(original_size)} → "
                        f"{self.format_size(new_size)})"
                    )
                else:
                    temp_path.unlink()
                    self.update_metadata_only(video_path)
                    self.skipped_count += 1
                    status = f"{Constants.YELLOW}SKIP{Constants.RESET}"
            else:
    ...
    def process_folder(self) -> None:
        videos = [
            ...
            if p.is_file() and p.suffix.lower() in Constants.VIDEO_EXTENSIONS
        ]
        ...
        for i, video in enumerate(videos, start=1):
            self.print_progress(i, len(videos), f"Checking {video.name}")
            try:
                res = subprocess.run(
                    [
                        "ffprobe",
        ...
        print(
            f"⏱️  Time Taken : "
            f"{self.format_time(time.time() - self.start_time)}\n"
        )


def main() -> None:
    print(f"{Constants.CYAN}🎬 Video Converter v2.3{Constants.RESET}")
